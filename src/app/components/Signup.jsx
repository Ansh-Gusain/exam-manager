...
export function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const user = await signUpWithCredentials(form.name, form.email, form.password, form.role);
        ...
        <Card className="border border-gray-700 shadow-2xl bg-gray-900 text-white">
          <CardHeader className="space-y-3 pb-6">
            ...
            <CardDescription className="text-center text-gray-400">
              Register for the Exam Manager portal
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input type="email" value={form.email} onChange={e => setForm({ ...form, email: e.target.value })}
                  placeholder="you@gbu.ac.in" required
                  className="bg-gray-800 border-gray-700 text-white placeholder:text-gray-500" />
              </div>
              <div className="space-y-2">
